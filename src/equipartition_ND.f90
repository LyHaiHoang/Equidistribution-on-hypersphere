...
SUBROUTINE equipartition_ND(nv, D, nat, nb_mat, nv_fix, sym_mat, perm_mat, init_algo, &
                            opt_algo, nstep_max, X, nnangl, dmean, dstddev, dmin, dmax, Etot )
!----------------------------------------------------------------------
!> @brief
...
!! a sorted list of vectors and their average distance between first neighbors
  ...
  INTEGER,                    INTENT(IN)    :: nstep_max      ! Maximum number of steps for the initialization
  ...
  IF (opt_algo==1 .OR. opt_algo==3) &
      CALL min_force(nv,D,nat,nb_mat,X,mat,nv_fix,sym_mat,perm_mat, Etot,Riesz, nstep_max)
  !
  ! ... Optimize the starting positions by minimizing dotproduct
  IF (opt_algo==2 .OR. opt_algo==3) &
      CALL min_dotproduct(nv, D, nat, nb_mat, X, nv_fix, sym_mat, perm_mat, nstep_max)
  !
...
!----------------------------------------------------------------------
SUBROUTINE min_force(nv,D,nat,nb_mat,X,mat,nv_fix,sym_matrices,perm_matrices, Etot, s, nstep_max)
!----------------------------------------------------------------------
!> @brief
!! Modifies the vectors positions on the hypersphere such that their interaction energy is minimum
  ...
  INTEGER,                    INTENT(IN)    :: nstep_max                 ! Max number of step for minimization 
  !
  ! ... Local Variables 
  ...
  DO WHILE ( .NOT. do_exit )
      ...
      PRINT*,nsteps,"E=",Etot 
      !'a=',alpha,'Fpmax=',MAXVAL(F_perp),'DE=',Etot_old-Etot,'DX=',MAXVAL(X-Xold),'try=',try 
      ! 
      ! ... Check if exit
      !do_exit = ((( (Etot_old-Etot)<eps .OR. MAXVAL(X-Xold)<(PI/4000.0)) &
      !            .AND. (try>3000) )                 &
      !            .OR. MAXVAL(F_perp)<0.00000000001) &
      do_exit = (nsteps>nstep_max) 
      nsteps  = nsteps+1
      !
...
!----------------------------------------------------------------------
SUBROUTINE min_dotproduct(nv,D,nat,nb_mat,X, nv_fix,sym_mat,perm_mat, nstep_max)
!----------------------------------------------------------------------
!> @brief
!! Modifies the vectors on the hypersphere such as to minimize the dot product between them
  ...
  INTEGER,                    INTENT(IN)    :: nstep_max      ! Maximum number of minimization steps
  !
  ! ... Local Variables 
  ...
  REAL(DP)                                  :: dpmax, dpmaxnew! Max value of dot product and its new value
  REAL(DP)                                  :: exit_thr       ! Angle threshold for exit
  REAL(DP)                                  :: alpha          ! Force factor for displacement
  REAL(DP), DIMENSION(:,:), ALLOCATABLE     :: dp             ! dot products matrix
  INTEGER                                   :: ndim           ! total dimension
  INTEGER                                   :: try, nsteps    ! Counters
  INTEGER                                   :: nvec           ! Total number of vectors
  INTEGER                                   :: i,j,k          ! Counters
  INTEGER                                   :: imax, imaxnew  ! Counters
  INTEGER                                   :: jmax, jmaxnew  ! Counters
  INTEGER, DIMENSION(2)                     :: loc            ! Coordinates of max dot product in the matrix
  LOGICAL                                   :: do_exit        ! Exit condition
  !
  ! ... Allocate tabulars
  ndim = nat*D
  ...
  DO WHILE ( .NOT. do_exit )
     !
     ! ... If the max dot product has been modified
     !IF (dpmaxnew<dpmax) THEN
        ! 
        ! ... Calculate all the dot products
        !OMP PARALLEL DO PRIVATE(i,j) SHARED(dp,x_)
        DO i=1, nvec
           DO j=i+1, nvec
              dp(i,j)= DOT_PRODUCT( x_(:,i), x_(:,j) )
           ENDDO
        ENDDO 
        !OMP END PARALLEL DO
        !
        ...
        imaxnew=loc(1)
        jmaxnew=loc(2)
        dpmaxnew=dp(imaxnew, jmaxnew)
        !DO i=1,nvec
        !   PRINT*,dp(i,:)
        !ENDDO
     !ENDIF
     PRINT *, 'alpha=', alpha, 'try=', try, 'dpmaxnew=',dpmaxnew, 'dpmax=', dpmax,'imax=', imax,'jmax=', jmax
     !
     ! ... Modify alpha according to the new dot produt 
     IF ( dpmaxnew >= dpmax )  THEN
     ...
     x_tmp(:)   = x_(:,imax)
     x_(:,imax) = x_(:,imax) -alpha*x_(:,jmax)
     x_(:,jmax) = x_(:,jmax) -alpha*x_tmp(:)
     x_(:,imax) = x_(:,imax)/NORM2(x_(:,imax))
     x_(:,jmax) = x_(:,jmax)/NORM2(x_(:,jmax))
     !DO i=1,nvec
     !   PRINT*, "Vect ",i," = ",x_(:,i)
     !ENDDO
     !
     !
     ! ... Check if exit
     !do_exit = ( (dpmax < exit_thr) .OR. (try>2000) .OR. (nsteps>nstep_max) )    
     nsteps  = nsteps+1
     do_exit = (nsteps>nstep_max)    
     !
  ENDDO
